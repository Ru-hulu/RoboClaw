...
async def record_episode(executor: Any, context: Any, skill: Any, episode_num: int, on_progress: Any = None) -> dict[str, Any]:
...
class EpisodeSupervisor:
    ...
    async def supervise_episode(self, executor: Any, context: Any, skill: Any, episode_num: int, on_progress: Any = None) -> tuple[dict[str, Any], EpisodeVerdict]:
        reset_result = await executor.execute_reset(context)
        episode = (
            await record_episode(executor, context, skill, episode_num, on_progress=on_progress)
            if reset_result.ok
